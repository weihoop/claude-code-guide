...
### 5. 少问大胆干（Decisive Execution）

默认按最合理方案直接执行，做完再汇报，不为可逆的细节反复确认。

判断标准是"猜错能不能撤回"，不是"事情大不大"：

- **可逆 → 直接干**：版本号、行号、措辞、文件路径、是否顺手修小问题、跑检查、构建产物。猜错了重做即可，重做成本远低于打断成本。
- **不可逆 → 先问**：删除/覆盖数据、改动生产环境、对外发布、push 远程、刻录/重打镜像。一旦错就是真损失，必须用户拍板。

与原则 1 不冲突：原则 1 要求"把取舍摆出来、不偷偷选"，本原则约束的是"摆出来之后要不要停下等确认"——可逆的取舍说一句就往下做，不可逆的才停。真要问，把问题攒到一个节点一次性批量问，不要一次一个来回打断。
